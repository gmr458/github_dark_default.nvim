...
M.config = {
    compile_path = vim.fn.stdpath("cache") .. "/github_dark_default",
...
vim.g.colors_name = "github_dark_default"
local h = vim.api.nvim_set_hl]]),
    ...
    local hgs = require("github_dark_default.highlight_groups").get(config)
    ...
    if not f then
        local err_path = (config.path_sep == "/" and "/tmp" or os.getenv("TMP")) .. "/github_dark_default_error.lua"
...
vim.api.nvim_create_user_command("GitHubDarkDefaultCompile", function()
    M.compile(M.config)
    vim.notify("github_dark_default colorscheme compiled")
